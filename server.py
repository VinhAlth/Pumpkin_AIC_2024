...
def get_eval_id():
    session_id = "8C6ZzJcKGo1FwKqkuC_LYQFxWKO8RLCG"
...
def index():
    ...
    if request.method == 'POST':
        ...
        for _, info in enumerate(lst_video_name):
            ...
            if len(se)==1: 
                se="0"+se
            video_info  = info['video_name'] + ", " + mi +":"+se + ', ' + str(int(fps))
        ...
        return render_template('index.html', files=files, shortened_files=shortened_files, query=text, image=image, count=str(len(files)) +' files found.' , s2t={}, list_frames = json.dumps(list_frames), model=model_port, scroll=scroll, k = k)
    else:
